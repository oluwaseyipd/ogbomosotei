...
import { HiOutlineRefresh, HiOutlineLogout, HiOutlineDownload } from 'react-icons/hi'
...
const TABLES = ['registrations', 'volunteers', 'sponsors', 'exhibitors', 'newsletter_subscribers']
...
export default function Admin() {
  ...
  const tabs = [
    ...
    { key: 'newsletter_subscribers', label: 'Newsletter', count: allData.newsletter_subscribers.length },
  ]
...
<div className="flex gap-0.5 mb-6 border-b border-gray-200 overflow-x-auto whitespace-nowrap scrollbar-hide scroll-smooth">
  {tabs.map(t => (
    <button 
      key={t.key} 
      // This ref callback runs whenever the activeTab changes
      ref={(el) => {
      ...
      className={`px-5 py-3 text-xs font-semibold tracking-wide transition-colors duration-150 relative flex-shrink-0 ${
        activeTab === t.key
    ...
    >
      ...
      <span className={`ml-2 px-1.5 py-0.5 text-[10px] font-bold rounded-sm ${activeTab === t.key ? 'bg-orange-50 text-brand-orange' : 'bg-gray-100 text-gray-400'}`}>
        ...
        <div className="bg-white border border-gray-100 p-6">
          ...
          {activeTab === 'newsletter_subscribers' && <NewsletterTab data={allData.newsletter_subscribers} loading={loading} />}
        </div>
      </div>
    </div>
  )
